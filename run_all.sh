...
# ── Run All Scripts ───────────────────────────────────────────
run_script "Script 1 — System Identity"      "$SCRIPT_DIR/script1.sh"
run_script "Script 2 — FOSS Package Inspector" "$SCRIPT_DIR/script2.sh"
run_script "Script 3 — Disk & Permission Auditor" "$SCRIPT_DIR/script3.sh"
run_script "Script 4 — Log Analyzer (syslog)" \
    <(echo '#!/bin/bash'; echo "bash \"$SCRIPT_DIR/script4.sh\" /var/log/syslog error")
run_script "Script 5 — Manifesto Generator"  "$SCRIPT_DIR/script5.sh"
