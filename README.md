# oss-audit-24BCE10812

# Open Source Audit Project — Git

## Student Details

**Name:** Ashish Ranjan Pandey
**Registration Number:** 24BCE10812
...
```bash
git clone https://github.com/your-username/oss-audit-24BCE10893.git
cd oss-audit-24BCE10893
```
...
./script2.sh
./script3.sh
./script4.sh /var/log/syslog [keyword]
./script5.sh
```
...
```bash
./script2.sh          # inspect git
./script2.sh firefox  # inspect any package
```
...
### 4. Log File Analyzer (`script4.sh`) ⚡ Performance Fix
...
```bash
./script4.sh /var/log/syslog          # searches 'error'
./script4.sh /var/log/syslog warning  # searches 'warning'
```
